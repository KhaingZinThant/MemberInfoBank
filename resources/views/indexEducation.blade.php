...
<div class="container">
  ...
  <table class="table table-striped table-borderless table-hover bg-light">
    ...
    <tbody>
     @foreach($educationVar as $e)
  <tr>
    <td><input type="checkbox" name="active" value="1"></td>
    <td>{{$e->educationDesc}}</td>
    <td>{{$e->active}}</td>
    <td>{{$e->remark}}</td>
    
    <td>
      <a href="{{route('educationCN.edit',$e->educationId)}}" class="glyphicon glyphicon-edit"></a>

    </td>
       

    <td>
    <form action="{{route('educationCN.destroy',$e->educationId)}}" method="Post">
      @method('DELETE')
