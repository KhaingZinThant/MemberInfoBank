...
<html>
<head>
    <title>Avoid Foods List Form</title>
...
<div class="container">
    <h1><center>Avoid Foods List Data</center></h1><br>
       <div class="pull-right">
   <a class="btn btn-success" href="{{route('avoidFoodCN.create')}}" > Print </a>
     <a class="btn btn-success" href="javascript:void(0)" id="createNewfood"> Create New Avoid Foods</a>
  </div>
            ...
            <div class="modal-body">
              <span id="form_result"></span>
                ...
                <form id="foodForm" name="foodForm" class="form-horizontal">
                   <input type="hidden" name="avoidFoodId" id="avoidFoodId">
                    <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">Name</label>
                        <div class="col-sm-12">
                            <input type="text" class="form-control" id="name" name="avoidFoodDesc" placeholder="Enter Avoid Food Name" value="" maxlength="50" required="">
                        </div>
                    </div>
                     <div class="form-group">
...
<script type="text/javascript">
  $(function () {
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('avoidFoodCN.index') }}",
        columns: [
            { className: "center",defaultContent: '<input type="checkbox">'},
            {data: 'avoidFoodId', name: 'avoidFoodId'},
            {data: 'avoidFoodDesc', name: 'avoidFoodDesc'},
    ...
    $('body').on('click', '.editFood', function () {
      ...
      $.get("{{ route('avoidFoodCN.index') }}" +'/' + food_id +'/edit', function (data) {
          $('#modelHeading').html("Edit food");
          $('#saveBtn').val("edit-user");
          $('#ajaxModel').modal('show');
          $('#avoidFoodId').val(data.avoidFoodId);
          $('#name').val(data.avoidFoodDesc);
   ...
   $('body').on('click', '.deleteFood', function () {
       ...
       $('#ok_button').text('Delete');
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('avoidFoodCN.store') }}"+'/'+food_id,
              beforeSend:function(){
